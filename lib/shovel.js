...
// TODO: hey I needed this
module.exports.compile = function compile(opts, cb, strategies)
{
    cb = cb || function (){};

    var copts = {compiling: true};
    for(var key in opts)
    {
        copts[key] = opts[key];
    }

    run(copts, strategies, function (buffer)
    {
...
function exec(opts, name, args, process_options, process_stdin, cb)
{
    ...
    function cleanupFiles()
    {
        ...
        if (opts.tempFiles)
        {
            ...
            for (var i = 0; i < opts.tempFiles.length; i++)
            {
                //fs.unlink(opts.tempFiles[i], cb);
            }
            //v TODO: this doesn't actually do anything important, right?
