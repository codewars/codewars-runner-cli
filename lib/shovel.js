...
function run(opts, strategies, cb)
{
    ...
    function executer(params, /* used only when params is a string: */ stdout, stderr, code, signal)
    {
        // if params is a string, consider it an error
        if (typeof params === 'string')
        {
            cb({
                ...
                exitCode: code,
                exitSignal: signal,
                ran: false // indicates that the code was not run (usually a compile error)
            });
        }
        ...
        // Happy Path: otherwise assume it is a config object used to run the actual code
        else
        {
