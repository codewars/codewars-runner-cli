...
module.exports.start = function start(opts, cb, strategies)
{
    ...
    if (opts.ably && opts.channel) {
        try {
            ...
            opts.publish = channel.publish.bind(channel);
        } catch(e)
        {}
    }
...
function run(opts, strategies, cb)
{
    function executer(params)
    {
        ...
        {
            exec(opts, params.name, params.args, params.options, params.stdin, params.compileStdout, cb);
        }
    }
...
function runShell(opts, resolve) {
    ...
    if (opts.shell) {
        ...
        exec(shellOpts, 'bash', [file], {}, null, null, function(result) {
...
function exec(opts, name, args, processOptions, processStdin, compileStdout, cb)
{
    ...
    if (compileStdout) {
        opts.publish('stdout', compileStdout);
    }
