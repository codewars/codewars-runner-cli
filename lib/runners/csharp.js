...
module.exports.run = function run(opts, cb)
{
    ...
    shovel.start(opts, cb,
    {
        compile:
        {
            ...
            {
                return {name: 'mcs', args: ['-langversion:4', '-target:library', '-lib:' + path.resolve(path.dirname(require.main.filename), nunitPath), '-r:' + nunitAssemblies, '-out:' + '/tmp/out.dll', file.path, fixtureFile.path]};
            }
        },
        solutionOnly: function ()
        {
        ...
        {
            opts.tempFiles = [outFile.path];
            process.env.MONO_PATH = path.resolve(path.dirname(require.main.filename), nunitPath);
