    ...
    shovel.start(opts, cb, {
        ...
        testIntegration: function (exec)
        {
            switch (opts.testFramework)
            {
                ...
                case 'mocha':
                case 'mocha_bdd':
                    return prepareMocha(opts, 'bdd', exec);

                case 'mocha_tdd':
                    return prepareMocha(opts, 'tdd', exec);

                case 'karma':
                case 'karma_bdd':
                    return prepareKarma(opts, 'bdd', exec);

                case 'karma_tdd':
                    return prepareKarma(opts, 'tdd', exec);
...
function prepareMocha(opts, interfaceType, exec) {
    var code = `
        ${opts.setup};
        ${opts.solution};
        (function() {
    ...
    var solutionFile = util.codeWriteSync('javascript', maybeTransform(code, opts), null, 'mocha', true);
    
    // NOTE: Mocha based testing currently does not support Node versioning
    exec({name: 'mocha', 'args': ['-t', opts.timeout || 8000, '-u', interfaceType, '-R', 'mocha-reporter', solutionFile]});
}

function prepareKarma(opts, interfaceType, exec) {
	var dir = '/workspace';
...
function execNode(opts, code, exec, requires) {
    var version = versionInfo(opts);
    code = maybeTransform(code, opts);

    var args = ['-e', code, '--no-deprecation', '--trace-warnings'];
