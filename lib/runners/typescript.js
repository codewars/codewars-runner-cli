...
const jsRunner = require('./javascript');
  ...
  function prepareMocha(opts, interfaceType, runCode, fail) {
    ...
    exec(cmd + ' ' + codeFile, function(error, stdout, stderr) {
      ...
      exec(cmd + ' ' + specFile, function(error, stdout, stderr) {
        ...
        runCode({name: 'mocha', 'args': ['-t', opts.timeout || 7000, '-u', interfaceType, '-R', '/runner/frameworks/javascript/mocha-reporter.js', specFileJS]});
      });
    });
  }
