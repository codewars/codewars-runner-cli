...
module.exports.run = function run(opts, cb) {
  shovel.start(opts, cb, {
    solutionOnly(runCode) {
      ...
      if (opts.setup)
        fs.outputFileSync(path.join(opts.dir, 'src', 'main', 'groovy', 'setup.groovy'), opts.setup);
      fs.outputFileSync(path.join(opts.dir, 'src', 'main', 'groovy', 'Main.groovy'), opts.solution);
    ...
    testIntegration(runCode) {
      ...
      if (opts.setup)
        fs.outputFileSync(path.join(opts.dir, 'src', 'main', 'groovy', 'Setup.groovy'), opts.setup);
      fs.outputFileSync(path.join(opts.dir, 'src', 'main', 'groovy', 'Solution.groovy'), opts.solution);
      fs.outputFileSync(path.join(opts.dir, 'src', 'test', 'groovy', 'Fixture.groovy'), opts.fixture);
      runCode({
        name: 'gradle',
        args: [
          process.env.GRADLE_DAEMON_FLAG || '--no-daemon',
